...
public class TheStack : MonoBehaviour
{
    ...
    public AudioClip break1, break2, highScore, perfect;
    public AudioSource audioSfx;

    public bool highScoreCheck = true;
    ...
    private void CreateRubble(Vector3 pos, Vector3 scale)
    {
        ...
        if (Random.Range(0, 2) == 0)
            audioSfx.PlayOneShot(break1,0.5f);
        else
            audioSfx.PlayOneShot(break2, 0.5f);
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            if (PlaceTile())
            {
                ...
                if (highScoreCheck && scoreCount > PlayerPrefs.GetInt("score"))
                {
                    audioSfx.PlayOneShot(highScore, 0.7f);
        ...
        if (isMovingOnX)
        {
            ...
            else
            {
                if(combo > COMBO_START_GAIN)
                {
                    ...
                    float middle = lastTilePosition.x + t.localPosition.x / 2;
                    t.localScale = new Vector3(stackBounds.x, 1, stackBounds.y);
                    t.localPosition = new Vector3(middle - (lastTilePosition.x / 2), scoreCount, lastTilePosition.z / 2);
                }
                combo++;

                audioSfx.PlayOneShot(perfect, 0.7f);
